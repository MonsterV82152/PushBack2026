...
void limelib::Chassis::moveToPointTask(Point2D point, int timeout, moveToPointParams params)
{
    ...
    while (!movementHelper.isDone())
    {
        ...
        real_t lateralOutput = velocityController.update(velocityOutput - locator.getVelocity().linear);
...
void limelib::Chassis::moveToPose(Pose2D pose, int timeout, moveToPoseParams params)
{
    ...
    movementTask.remove();
    movementTask.create([this, pose, timeout, params]()
                        { moveToPoseTask(pose, timeout, params); }, "LIMELIB_MOVE_TO_POSE");
}

void limelib::Chassis::moveToPoseTask(Pose2D pose, int timeout, moveToPoseParams params)
{
    // Stuff
}
