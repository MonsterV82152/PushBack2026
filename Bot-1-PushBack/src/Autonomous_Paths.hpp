...
void skills()
{
    ...
    { // Clearing Red Park zone
        correct_position(leftLoc, &chassis, true);
        chassis.moveToPoint(-65, 28, 2300, {.maxSpeed = 90});
        rollers::setState("intake");
        chassis.waitUntilDone();
        correct_position(leftLoc, &chassis, true);
    ...
    { // Matchload Clearing - First
        ...
        matchLoader.setState(true);
        chassis.turnToPoint(-70, 60, 700);
        chassis.moveToPoint(-70, 60, 1000, {.minSpeed = 127});
        chassis.waitUntilDone();
        for (int i = 0; i < 6; i++)
        {
            chassis.arcade(-20, 0);
    ...
    { // Traveling to 2nd Match Load
        chassis.turnToPoint(-36, 58, 700);
        chassis.moveToPoint(-36, 58, 700, {.minSpeed = 90, .earlyExitRange = 4});
        chassis.turnToPoint(38, 70, 700, {.minSpeed = 90, .earlyExitRange = 15});
        chassis.moveToPoint(38, 70, 1000, {.minSpeed = 90, .earlyExitRange = 2});
        pros::delay(200);
        correct_position(leftLoc, &chassis, false);
        chassis.moveToPoint(40, 46, 700);
        matchLoader.setState(true);
        chassis.turnToPoint(70, 48, 700);
    }
    ...
    { // Clearing 2nd Matchload
        ...
        chassis.waitUntilDone();
        correct_position(leftLoc, &chassis, false);
        chassis.moveToPoint(70, 48, 1000, {.minSpeed = 127});
        chassis.waitUntilDone();
        for (int i = 0; i < 6; i++)
        {
            chassis.arcade(-20, 0);
        ...
        correct_position(leftLoc, &chassis, false);
        chassis.moveToPoint(40, 46, 1200, {false});
        chassis.waitUntilDone();
        pros::delay(1000);
        matchLoader.setState(false);
    ...
    { // Drive through park zone
        chassis.moveToPoint(60, 28, 1000,{.minSpeed = 90, .earlyExitRange = 4});
        chassis.swingToPoint(70,-28,lemlib::DriveSide::RIGHT,700,{.minSpeed = 90, .earlyExitRange = 4});
        correct_position(leftLoc, &chassis, true);
        chassis.moveToPoint(70, -28, 2000, {.maxSpeed = 80});
        rollers::setState("intake");
        chassis.waitUntilDone();
        correct_position(leftLoc, &chassis, true);
...

    {
        ...
        correct_position(leftLoc, &chassis, true);
        chassis.turnToPoint(0,0,700);
        chassis.moveToPoint(16, -16, 1000, {.maxSpeed = 40});
        pros::delay(600);
        ...
        chassis.moveToPoint(-70, 0, 1000, {.minSpeed = 80});
    }
